Shader "Bigi/AudioLink_fragv8" {
	Properties {
		...
		[Header(Decals)]
		[TOGGLE(DECALS_ENABLED)] _DecalEnabled ("Enable Decals",Float) = 0.0
	...
	SubShader {
		...
		Pass {
			...
			fragOutput frag(v2f i)
			{
				fixed4 orig_color = UNITY_SAMPLE_TEX2D(_MainTex, i.uv);
					...
					fixed4 decal_color = UNITY_SAMPLE_TEX2D(_DecalTex,i.uv);
					orig_color = lerp(orig_color,decal_color,decal_color.a);
				#endif
		...
		UsePass "Bigi/AudioLink_fragv7/Outline"

		//UsePass "Legacy Shaders/VertexLit/SHADOWCASTER"
		UsePass "Bigi/AudioLink_fragv7/ShadowPass"
	}
}